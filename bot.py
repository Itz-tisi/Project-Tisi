...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id == '1525613151':  # Updated with your admin ID
        ...
        if len(command) == 2:
            try:
                ...
                response = f"Please specify a valid number and unit of time (hours/days)."
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = f"ONLY OWNER CAN USE."
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            response = f"Invalid or expired key."
    else:
...
def main() -> None:
    application = ApplicationBuilder().token('7461925686:AAHiQp1RS7YAVFVVHoWEyKgaE5wGYgO0QJo').build()  # Updated with your bot token
